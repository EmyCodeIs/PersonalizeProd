...
function splitConfigured(value) {
  return String(value || '')
    .split(/[;,\n]/)
    .map((item) => item.trim())
    .filter(Boolean);
...
function configuredTesterIdentities() {
  return [...new Set([
    ...splitConfigured(process.env.TEST_COMMAND_ALLOWED_CLIENT_NUMBERS),
    ...splitConfigured(process.env.TEST_COMMAND_ALLOWED_CHAT_IDS),
    ...splitConfigured(process.env.ADMIN_WHATSAPP_NUMBERS),
    ...splitConfigured(process.env.ADMIN_WHATSAPP_CHAT_IDS),
  ])];
}
