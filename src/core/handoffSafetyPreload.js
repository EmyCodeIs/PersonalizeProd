...
const historyInspectionCache = new Map();
...
function invalidateHistoryInspection(clientId) {
  historyInspectionCache.delete(contactKey(clientId));
}
...
function activateHandoff(clientId, payload = {}) {
  ...
  invalidateHistoryInspection(clientId);
...
function visibleOutgoing(message = {}) {
  if (!message?.fromMe) return false;
  const text = String(message?.body || message?.caption || message?.text || '').trim();
  ...
  return Boolean(text || filename || /image|video|document|pdf|application|audio|ptt|sticker/.test(type));
}
...
async function readConversationHistory(client, clientId) {
  ...
  return { available, chatId: ids[0] || null, messages: [] };
}

async function inspectHistoricalHumanControl(channel, clientId, { force = false } = {}) {
  const key = contactKey(clientId);
  const cached = historyInspectionCache.get(key);
  if (!force && cached && (Date.now() - cached.at) <= HISTORY_CACHE_TTL_MS) return cached.value;

  const history = await readConversationHistory(channel?.client, clientId);
  if (!history.available) {
    const value = { blocked: false, conclusive: false, reason: 'historico_indisponivel' };
    historyInspectionCache.set(key, { at: Date.now(), value });
    return value;
  }

  const messages = [...history.messages].sort((left, right) => {
    const a = extractTimestampMs(left);
    const b = extractTimestampMs(right);
    if (a === null || b === null) return 0;
    return a - b;
  });

  const checkpoint = HistoryPolicy.withHandoffHistoryBoundary(
    () => BotActivity.getLastBotOutbound(clientId),
  );
  ...
  if (checkpointIndex >= 0) {
    outgoing = messages.slice(checkpointIndex + 1).filter(visibleOutgoing);
  } else if (Number.isFinite(checkpointAt) && checkpointAt > 0) {
    outgoing = messages.filter((message) => {
      if (!visibleOutgoing(message)) return false;
      const timestamp = extractTimestampMs(message);
      return timestamp !== null && timestamp > (checkpointAt + 1500);
    });
  }

  const value = outgoing.length
    ? {
        blocked: true,
        conclusive: true,
    ...
    : {
        blocked: false,
        conclusive: true,
        reason: 'sem_saida_humana_apos_checkpoint',
        chatId: history.chatId,
      };
  historyInspectionCache.set(key, { at: Date.now(), value });
  return value;
...
async function getAutomationBlock(channel, clientId) {
  ...
  try {
    const historical = await inspectHistoricalHumanControl(channel, clientId);
...
module.exports = {
  ...
  inspectHistoricalHumanControl,
  invalidateHistoryInspection,
  invalidateLabelInspection,
  registerManualTakeover,
  _test: {
    handoffBlockedError,
    historyInspectionCache,
