...
async function executeSystemReset({
  ...
  if (!env.enableTestCommands) {
    DecisionLog.log('ADMIN', 'reset_ignorado', {
      chat: clientId,
      motivo: 'ENABLE_TEST_COMMANDS=false',
    }, 'warn');
  ...
  if (!access.allowed) {
    DecisionLog.log('ADMIN', 'reset_negado', {
      chat: clientId,
  ...
  if (isHistoricalCommand({ raw, source })) {
    DecisionLog.log('ADMIN', 'reset_histórico_ignorado', {
      chat: clientId,
      msg: messageId(raw) || undefined,
      origem: source,
    });
  ...
  if (commandId && processedCommandIds.has(commandId)) {
    DecisionLog.log('ADMIN', 'reset_duplicado_ignorado', {
      chat: clientId,
      msg: commandId,
      origem: source,
    });
  ...
  if (activeReset) {
    DecisionLog.log('ADMIN', 'reset_em_andamento', { chat: clientId, origem: source });
    await activeReset;
  ...
  activeReset = (async () => {
    ...
    DecisionLog.log('ADMIN', 'reset_iniciado', {
      chat: clientId,
      origem: source,
      autorização: access.reason,
    ...
    DecisionLog.log('NOTA', contact.note.cleared ? 'limpeza_confirmada' : 'limpeza_falhou', {
      chat: clientId,
      encontradas: contact.note.found,
      erro: contact.note.error || undefined,
    }, contact.note.cleared ? 'info' : 'warn');
    DecisionLog.log('ETIQUETA', contact.labels.remaining === 0 ? 'limpeza_confirmada' : 'limpeza_falhou', {
      chat: clientId,
    ...
    DecisionLog.log('ADMIN', 'reset_concluído', {
      chat: clientId,
      resultado: ok ? 'OK' : 'PARCIAL',
      sessões: internal.previousSessionCount,
